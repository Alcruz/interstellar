...
We also conducted convergence analysis for the PSOR method. For this, we set up our experiment similar to how we described in \eqref{sec:finitediferencesschemes:numericalresults:convergenceanalysis}. Therefore, we mostly present the results in this section. As you observe in figures \eqref{fig:lcp:numericalresults:convergence_space_explicit}, and \eqref{fig:lcp:numericalresults:convergence_time_explicit} is order two in the spatial domain. We have similar bounds for Crank-Nicholson and implicit schemes. That the Crank-Nicholson is order two in the temporal domain. This would increase the converges significantly. However, by decreasing $\Delta{x}$ would, solving the LCP would take ages.
\begin{figure}[tbp]
  \centering
  \begin{subfigure}{0.4\textwidth}
    \centering
